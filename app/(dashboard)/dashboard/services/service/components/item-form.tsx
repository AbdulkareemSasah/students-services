...
interface CategoryFormProps {
    ...
    categoriesService:ItemType[]
}
const ItemForm : React.FC<CategoryFormProps> = ({initialData, categoriesService}) => {
    ...
    const name = "service"
    const head = {
        title: initialData ? initialData.translations.find(t => t.lang === selectedLang)?.title :t(`service.create.title`),
        description : initialData ? initialData.translations.find(t => t.lang === selectedLang)?.description :t(`service.create.description`),
    };
    const title = initialData ? t(`service.edit.title`) :t(`service.create.title`);
    const description = initialData ? t(`service.edit.description`) :t(`service.create.description`);
    const toastMessage = {
        success : initialData ? t(`service.toast.update.success`) : t(`service.toast.create.success`),
        error : initialData ? t(`service.toast.update.error`) : t(`service.toast.create.error`),
    };
    ...
    const defaultValues = initialData ? {
        categoryServiceId:initialData.categoryServiceId,
    ...
    const onSubmit = async () => {
        ...
        const formData = {
            ...
            categoryId:form.getValues().categoryServiceId
            };
        ...
        if (initialData) {
            try {
                ...
                if (response.ok) {
                    router.push(`/dashboard/service/${params?.id}#view`)
    ...
    const onDelete = async () => {
        try {
            setLoading(true);
            await axios.delete(`/api/service/${params?.id}`);
            router.refresh();
            router.push(`/dashboard/service`);
            toast.success(t(`service.toast.delete.success`));
        } catch (error: any) {
            toast.error(t(`service.toast.delete.error`));
        } finally {
            setLoading(false);
            setOpen(false);
        }
                                ...
                                return (
                                    <div key={l.language} className={selectedLang === l.language ? "gap-y-3" : "hidden"}>
                                        <FormField
                                            ...
                                            render={({ field }) => (
                                                <FormItem>
                                                    <FormLabel>{t(`service.form.name.label`)}</FormLabel>
                                                    <FormControl>
                                                        <Input disabled={loading} placeholder={t("service.form.name.placeholder")} {...field} />
                                                    </FormControl>
                                                    <FormMessage />
                                                </FormItem>
                                            )}
                                        ...
                                        <FormField
                                            ...
                                            render={({ field }) => (
                                                <FormItem>
                                                    <FormLabel>{t(`service.form.description.label`)}</FormLabel>
                                                    <FormControl>
                                                        <Input disabled={loading} placeholder={t(`service.form.description.placeholder`)} {...field} />
                                                    </FormControl>
                                                    <FormMessage />
                                                </FormItem>
                                            )}
                                            ...
                                            render={({ field }) => (
                                                <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                                                    ...
                                                    <div className="space-y-1 leading-none px-3">
                                                        <FormLabel>
                                                        {t(`service.form.published.label`)}
                                                        </FormLabel>
                                                        <FormDescription>
                                                        {t(`service.form.published.description`)}
                                                        </FormDescription>
                                                    </div>
                                                </FormItem>
                                            )}
                                        ...
                                        <FormField
                                            ...
                                            render={({ field }) => (
                                                <FormItem>
                                                    <FormLabel>{t("service.form.blocks.label")}</FormLabel>
                                                    <DynamicForm
                                                        field={field}
                                                        loading={loading}
                                                    />
                                        ...
                                        <FormField
                                            ...
                                            render={({ field }) => (
                                                <FormItem>
                                                    <FormLabel>{t("service.form.category.label")}</FormLabel>
                                                    <Select disabled={loading} onValueChange={field.onChange} value={field.value} defaultValue={field.value}>
                                                        <FormControl>
                                                            <SelectTrigger className='w-fit min-w-[8em]'>
                                                                <SelectValue defaultValue={field.value} placeholder={t("service.form.category.placeholder")} />
                                                            </SelectTrigger>
                                                        </FormControl>
                                                        <SelectContent className='w-fit min-w-[8em]'>
                                                            {categoriesService.map((color) => (
